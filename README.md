### Olá, sou o Matheus Rocha 👋
### Acadêmico de Engenharia da Computação | Entusiasta de Linux & Baixo Nível
<div>
<a href="https://www.linkedin.com/in/matheus-rocha-canto-8b071a300" target="_blank"><img loading="lazy" src="https://img.shields.io/badge/-LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" target="_blank"></a>
<a href = "mailto:rochacanto@gmail.com"><img loading="lazy" src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" target="_blank"></a>
<a href="https://instagram.com/matteo_rcw" target="_blank"><img loading="lazy" src="https://img.shields.io/badge/-Instagram-%23E4405F?style=for-the-badge&logo=instagram&logoColor=white" target="_blank"></a>
</div>


## Hi there, take your time!

#### Sou um desenvolvedor focado em lógica e resolução de problemas complexos. Com base em Manaus (AM), atuo com foco em **Pesquisa e Desenvolvimento (P&D)** e **Linux composto em Desktop e Hardware**.

🧠 **Neurodiversidade como Potência:** Como profissional neurodivergente (TEA/TDAH), utilizo minha capacidade de **hiperfoco e atenção a detalhes** para investigar falhas, documentar processos e validar a lógica de sistemas.

#### 🚀 Experiência Principal
*   **Pesquisador Bolsista (Projeto SUPER/Samsung):** Desenvolvi um "Computador de Voo" completo em **Dart/Flutter** (Mobile) ao longo de 1 ano.
*   #### 🏗️ Estruturas de Dados (C)
Implementação de dicionários dinâmicos e manipulação direta de memória, focando em eficiência computacional

## Tecnologias e ferramentas
*   **OS:** Linux (Ubuntu/Debian e Arch), Windows.
*   **Linguagens:** C, Python, Shell Script, Dart (Lógica).
*   **Ferramentas:** Git, GCC, Terminal.
*   **Idiomas:** Inglês Nativo/C2 (Apto para leitura de documentação técnica complexa)

[Link para meu LinkedIn](https://www.linkedin.com/in/matheus-rocha-canto-8b071a300/)
